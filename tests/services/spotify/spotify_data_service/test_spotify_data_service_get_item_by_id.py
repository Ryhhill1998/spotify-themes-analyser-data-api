...
@pytest.mark.asyncio
async def test__get_item_by_id_raises_spotify_data_service_unauthorised_exception_if_endpoint_requester_unauthorised_exception_occurs(
...
@pytest.mark.asyncio
async def test__get_item_by_id_item_not_found(
        spotify_data_service,
        mock_endpoint_requester
):
...
@pytest.mark.asyncio
async def test__get_item_by_id_request_failure(spotify_data_service, mock_endpoint_requester):
...
@pytest.mark.asyncio
async def test__get_item_by_id_calls_endpoint_requester_get_method_with_expected_params(
        spotify_data_service,
        mock_endpoint_requester
):
    await spotify_data_service._get_item_by_id(access_token="access", item_id="1", item_type=SpotifyItemType.ARTIST)
...
@pytest.mark.asyncio
async def test__get_item_by_id_returns_expected_data(spotify_data_service, mock_endpoint_requester):
    mock_data = {"test_key": "test_value"}
    mock_endpoint_requester.get.return_value = mock_data

    data = await spotify_data_service._get_item_by_id(
        access_token="access",
        item_id="1",
...
async def test_get_artist_by_id_calls_expected_methods(spotify_data_service):
    mock__get_item_by_id = AsyncMock()
    mock__create_artist = Mock()
    spotify_data_service._get_item_by_id = mock__get_item_by_id
    ...
    mock__get_item_by_id.assert_called_once_with(access_token="access", item_id="1", item_type=SpotifyItemType.ARTIST)
...
async def test_get_artist_by_id_calls_expected_methods(spotify_data_service):
    ...
    mock__get_item_by_id.assert_called_once_with(access_token="access", item_id="1", item_type=SpotifyItemType.TRACK)
